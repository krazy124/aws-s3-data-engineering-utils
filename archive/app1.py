    ...
    if st.button("Run MonsterForge ETL", type="primary"):
        try:
            ...
            with st.spinner("Loading raw data with PySpark..."):
                raw_df = monsterforge.load_monsterforge_raw_data(spark, temp_input_path)
            ...
            with st.spinner("Running MonsterForge cleaning pipeline..."):
                cleaned_df = monsterforge.monsterforge_cleaning_pipeline(raw_df)
                clean_df, quarantine_df = monsterforge.split_clean_quarantine(cleaned_df)
                report = monsterforge.generate_quality_report(cleaned_df, clean_df, quarantine_df)
            ...
            if all(
                ...
                st.session_state.run_id = run_id
                st.session_state.clean_s3_key = clean_key
                st.session_state.quarantine_s3_key = quarantine_key
...
def show_s3_browser_page():
    st.title("🗂️ S3 Browser")

    prefix = st.text_input(
        "S3 prefix",
        value="",
        placeholder="Example: clean/monsters/",
    )

    if st.button("List Files"):
        files = s3.list_files(BUCKET_NAME, region=REGION)
        ...
        if files:
            ...
            st.dataframe(pd.DataFrame({"s3_key": files}), use_container_width=True)
        else:
